...
function smarty_function_sugar_phone($params, &$smarty)
{
    ...
    if (isset($params['value'])) {
        $hidenum = substr($params['value'],5);
       // $number = str_replace('/^\+1(\d{3})(\d{3})(\d{4})$/i', '$1-$2-$3', $params['value']);
       // $number = str_replace(array(' '), '', $params['value']);
	 $number = preg_replace(array(' ~\D~'), '', $params['value']);
        $phoneNumber = $params['value'];
	$capnum = substr($number , -2);
        $zoiperVersion = "5.5.14";
        //$masknum =  str_replace(range(0,9), "*", substr($number, 0, -3)) .  substr($number, -3);
	$masknum =  substr_replace($number,str_repeat('*',strlen($number) -3 - 2),
                    2,
                    strlen($number) - 3- 2);
        //$zoiperProtocol = "zoiper:" . $zoiperVersion . "://call?";
        //$zoiperProtocol = "zoiper:" . $zoiperVersion . "://call?";
        //$callString = $zoiperProtocol . "number:" . $phoneNumber;
        //appying hash on this part 
        $enc_num = hash('md5', $params['value'] );
        //endhash
        //masking
        $script= ' <script>
            $(document).ready(function(){
                setTimeout(function(){
                    $("a[href]#hideData").each(function(){
                        var href = this.href;
       ...
       $(function(){
        $(".mymodal").click(function(e){
            ...
            let text = $(this).data("phone");
            let mask = $(this).data("masked");
            let result =  mask;
            let fetch  = $(this).closest("tr").find("a").text();
            let action = $(this).data("action");
            let name = "Caller Name";
            var tamparos = $(this).closest("tr").find("a").attr("href");
            $("#myModal").modal("show");
            $(".dialmsg").text("Calling "+ fetch + ": ("+ result + ")");
          
		if (tamparos === undefined){
                window.location.href = "sip:" + text;
                return false;
                }
                window.location.href = "sip:" + text;
                window.location.href = tamparos;	
	         
      	  });
       });
        </script>';       
	  $phone_number = $number ; // Replace with the phone number you want to dial
         $call_string = "sip:$phone_number";//sip
  	 //$call_string="$phone_number ";//asterisk 
          $login_image_url = SugarThemeRegistry::current()->getImageURL('telepono-call.png');
	   ...
	   padding: 20px 16px; 
          ">
               ...
               <h4 class="modal-title">Call Notice</h4>
            </div>
          ...
          <a href="javascript:;" id="hideData" class="mymodal"  data-key="'.$key.'"data-masked="'.$masknum.'" data-module="'.$modulename.'" data-phone="'.$phone_number.'"><img src="'.$login_image_url.'" alt="call" width="16" height="16"></a>
          '.$masknum.''.$script;  
    ...
    //return header("Location: $call_string"); 
}
